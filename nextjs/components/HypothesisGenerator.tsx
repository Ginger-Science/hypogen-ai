...
import { Loader2, Brain, Sparkles, Link, Download, FileText, File } from "lucide-react";
...
export const HypothesisGenerator = () => {
  ...
  const { writeContractAsync, isMining } = useScaffoldWriteContract({ contractName: "Hypo" });
  ...
  const mintHypothesisNFT = async () => {
    if (!hypothesis || !connectedAddress) return;
    const notificationId = notification.loading("Uploading metadata to IPFS...");
    try {
      // 1. Build metadata
      const metadata = {
        name: `Hypothesis: ${hypothesis.hypothesis_text.slice(0, 32)}...`,
        description: hypothesis.hypothesis_text,
        external_url: "https://your-app-url.com", // Optional
        image: "https://your-default-image-or-generated-image-url", // Optional
        attributes: [
          { trait_type: "Confidence Score", value: hypothesis.confidence_score },
          { trait_type: "Key Insights", value: hypothesis.key_insights.length },
        ],
 ...
 yOffset += lineHeight * 1.5; // More space after references
  ...
  const exportToJSON = () => {
    ...
    const jsonData = JSON.stringify(hypothesis, null, 2);
    const blob = new Blob([jsonData], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `hypothesis_${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
    
    toast({
      title: "JSON Export Complete!",
      description: "Hypothesis exported as JSON file",
    });
  };
      ...
      {hypothesis && (
        <Card className="border-orange-200 dark:border-orange-800">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex gap-2">
              <Button 
                onClick={mintHypothesisNFT}
                disabled={!hypothesis || isMining}
